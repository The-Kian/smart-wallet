...
export default function LoginScreenWeb() {
  ...
  useEffect(() => {
    const handleTokenResponse = async (access_token: string) => {
      try {
        ...
        if (userProfile) {
          ...
          Alert.alert("Authentication Failed", "Could not fetch user profile from Google.");
        }
      } catch {
        Alert.alert("Authentication Failed", "Could not complete Google Auth login.");
      }
    };
    ...
    if (response?.type === "success") {
      ...
      Alert.alert("Authentication Failed", response.params.error || "Could not complete login.");
    }
  }, [response, signIn]);
  ...
  const handleMockSignIn = () => {
    signIn({
      ...
      avatarUrl: "https://ui-avatars.com/api/?name=Kian+Popat&background=0D8ABC&color=fff&size=256",
    });
  };
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: "#FAFAFA", justifyContent: "space-between", paddingVertical: 80, paddingHorizontal: 24 },
  brandingSection: { alignItems: "center", marginTop: 60 },
  logo: { fontSize: 42, fontWeight: "900", letterSpacing: -1, color: "#111111" },
